...
1. In the client-side code, you don’t need to explicitly set the client’s IP address. The client uses a dynamically assigned IP address from its own network interface, which is automatically assigned by the operating system.
    ->The client simply creates a socket and connects to the server by specifying the server's address (IP and port).
    ->When the connect() system call is used, the operating system automatically assigns a local IP address and port for the client-side of the connection. This is typically a free port chosen by the OS.
